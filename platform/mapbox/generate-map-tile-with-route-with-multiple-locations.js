/*******************************************
* 
* This script generates a route map image using Mapbox API based on given multiple postcodes that are comma-separated.
* 
* License: GNU GPLv3
...
exports.main = async (event, callback) => {
    ...
    try {
        ...
        coordinatesArray.forEach((coords, index) => {
            markers += `pin-l-${index + 1}+ff0000(${coords.join(',')})`;
            if (index < coordinatesArray.length - 1) markers += ',';
        });

        const staticMapUrl = `https://api.mapbox.com/styles/v1/mapbox/streets-v11/static/${markers},path-5+41a5dd-1(${encodeURIComponent(polyline)})/auto/300x300?access_token=${mapboxAccessToken}`;
